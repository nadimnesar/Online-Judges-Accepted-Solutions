...
void solve(){
    ...
    for(ll i = 1; i <= 10000; i++){
        presave[i] = Int.multiplication(presave[i-1], 2);
    }
}
    ...
    while(cin >> ch){
        if(ch != '#') binary += ch;
            ...
            for(ll i = binary.size()-1, j = 0; i >= 0; i--, j++){
                if(binary[i] == '1') decimal = Int.add(decimal, presave[j]);
            }
            ll remainder = 0;
            for(ll i = 0; decimal[i]; i++){
                remainder = ((remainder*10)+((ll)(decimal[i]-48)))%131071;
            }
            if(remainder) cout << "NO" << endl;
            else cout << "YES" << endl;
            binary.clear();
        }
    }
