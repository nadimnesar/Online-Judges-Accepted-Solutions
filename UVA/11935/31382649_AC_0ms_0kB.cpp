...
node cmd_maker(string str){
    ...
    ++i; bool flag = false;
    for(; str[i]; i++){
        if(str[i] >= '0' and str[i] <= '9'){ flag = true; break; }
        else x.c += str[i];
    }
    if(flag){
...
bool checker(double volume){
    ...
    for(int i = 0; i < n; i++){
        ...
        maxi = max(tank, maxi);
        pos = rode[i].p;
        if(rode[i].c == "Fuel consumption") fuel = rode[i].f;
        else if(rode[i].c == "Leak") leak++;
        else if(rode[i].c == "Gas station") tank = 0;
        else if(rode[i].c == "Mechanic") leak = 0;
    }
