...
ll quary(int index, int left, int right){
    ...
    if(left >= i and right <= j){
        ...
        ll up = upper_bound(full(segTree[index]), v) - segTree[index].begin();
        ll low = lower_bound(full(segTree[index]), v) - segTree[index].begin();

        if(low == segTree[index].size()) return 0;
        if(segTree[index][low] != v)  return 0;
