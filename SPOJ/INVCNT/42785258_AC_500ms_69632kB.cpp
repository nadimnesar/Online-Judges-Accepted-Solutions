...
ll quary(int index, int left, int right) {
  ...
  if (left >= i and right <= j) {
    auto it = lower_bound(full(segTree[index]), v);
    if (it == segTree[index].end()) return segTree[index].size();
    return it - segTree[index].begin();
  }
...
void solve() {
  ...
  for(ll i = 0; i < 4*maxx; i++) segTree[i].clear();
